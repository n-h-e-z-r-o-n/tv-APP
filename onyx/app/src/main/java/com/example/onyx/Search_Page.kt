...
class Search_Page : AppCompatActivity() {
    ...
    private fun setupSearchFunctionality() {
        ...
        /*
        // Optional: You can also add a click listener to the search icon if needed
        ...
        CoroutineScope(Dispatchers.IO).launch {
                try{
                        ...
                        if (query.isNotEmpty()) {
                            ...
                            val url = "https://api.themoviedb.org/3/search/multi?include_adult=true&query=$query"
                            ...
                            for (i in 0 until moviesArray.length()) {
                                ...
                                when (mediaType) {
                                    "person" -> {
                                        title = current.optString("name", current.optString("original_name", "Unknown"))
                                        imgUrl = "https://image.tmdb.org/t/p/w500" + current.optString("profile_path", "")
                                    ...
                                    "tv" -> {
                                        title = current.optString("name", current.optString("original_name", "Unknown"))
                                        date = current.optString("first_air_date").takeIf { it.isNotEmpty() }?.substring(0, 4) ?: ""
                                        imgUrl = "https://image.tmdb.org/t/p/w500" + current.optString("poster_path", "")
                                    ...
                                    "movie" -> {
                                        ...
                                        date = current.optString("release_date").takeIf { it.isNotEmpty() }?.substring(0, 4) ?: ""
                                        imgUrl = "https://image.tmdb.org/t/p/w500" + current.optString("poster_path", "")
                                        poster = current.optString("poster_path", "null")
...
data class SearchResult(
    val id: Int,
    val title: String,
    val type: String, // "movie", "show", or "person"
